...
export function ExpenseForm({ type, onSubmit, buttonText, gradient }: ExpenseFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await onSubmit({
        ...
        ...(type === 'regular' ? { payment_date: formData.paymentDate } : { date: new Date().toISOString().split('T')[0] })
      })
      
      if (result) {
        setFormData({ description: '', amount: '', category: 'otros', paymentDate: 1 })
        addToast({
          type: 'success',
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <input
        ...
        placeholder={type === 'regular' ? 'Descripción del gasto regular' : '¿En qué gastaste?'}
